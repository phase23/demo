...
public class FingerprintActivity extends AppCompatActivity implements FingerprintListener, OnClickListener {
    ...
    public void onEnrollmentProgress(int fingerId, int remaining, int reason) {
        if (reason == 0 && remaining == 0) {
            showLog("Fingerprint ID:" + fingerId + "  Enrollment SUCCESSFUL!");
        } else {
    ...
    public void onGetImageComplete(int result, byte[] imgBuff) {
        //if (!HCBoolean.isEmpty(result) && "00".equals(result)) {
        if (result == 0) {
            try {
    ...
    public void onGetImageFeature(int i, byte[] bytes) {
        showLog("onGetImageFeature: " + i + "\t" + StringUtils.convertBytesToHex(bytes));
    }

    @Override
    public void onGetImageISOFeature(int i, byte[] bytes) {
        showLog("onGetImageISOFeature: " + i + "\t" + StringUtils.convertBytesToHex(bytes));
    }
