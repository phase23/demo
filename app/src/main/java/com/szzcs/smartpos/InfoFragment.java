...
public class InfoFragment extends PreferenceFragment {
    ...
    private SwitchPreference mBtnTransMode;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                // base sdk version
                int resBaseVer = mBaseSysDevice.getBaseSdkVer(mBaseSdkVer);
                ...
                // sdk ver
                mSdkVer = mBaseSysDevice.getSdkVersion();
                if (mSdkVer != null)
                    mHandler.sendEmptyMessage(MSG_VER);

                // firmware ver
                int resFireVer = mBaseSysDevice.getFirmwareVer(mFirmwareVer);
                ...
                //    sn[i] = (byte) 0x33;
                //}
                //   mBaseSysDevice.setCustomSn(new String(sn));
                int resSN = mBaseSysDevice.getCustomSn(mCustomerSN);
                ...
                Log.e(TAG, "getSpStatus: " + mPosSecurity[5]);
              ...
              /*  int resMachine = mBaseSysDevice.getDevName(mMachineModel);
                Log.e(TAG, "Read sn res: " + resSN);
                if (resSN == SdkResult.SDK_OK) {
                ...
                if (getInfo == SdkResult.SDK_OK) {
                    ...
                    Log.e(TAG, "getDeviceInfo: " + getInfo + "\t" + len + "\t" + mImei);
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        findPreference(getString(R.string.key_show_log)).setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                mBaseSysDevice.showLog((Boolean) newValue);
                return true;
            }
        });
        // init sdk
        ...
        mBtnTransMode = (SwitchPreference) findPreference(getString(R.string.key_trans_mode));
        mBtnTransMode.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
    ...
    private boolean isTransMode() {
        ...
        Log.e(TAG, "isTransMode: " + isTransMode);
        return isTransMode;
    }
    ...
    private int openTransMode() {
        ...
        int ret = mBaseSysDevice.setExternalPort((byte) 0x01, 0, pubKey);
        return ret;
    }
    ...
    private int closeTransMode() {
        byte[] pubKey = new byte[5];
        int ret = mBaseSysDevice.setExternalPort((byte) 0x00, 0, pubKey);
        return ret;
    }
    ...
    private void sendData() {
        ...
        //    sb.append(cmd);
        //}
        //Log.e(TAG, "sendData: " + sb);
        //Log.e(TAG, "sendData: " + sb.length());
        //byte[] bytes = StringUtils.convertHexToBytes(sb.toString());
        int sendRet = mBaseSysDevice.externalPortSend(bytes.length, bytes);
        Log.e(TAG, "sendData sendRet: " + sendRet);
    }

    private void recvData() {
        int[] len = new int[1];
        byte[] data = new byte[2048];
        int recvRet = mBaseSysDevice.externalPortRcv(len, data);
        Log.e(TAG, "recvData recvRet: " + recvRet);
        byte[] subByte = StringUtils.subByte(data, 0, len[0]);
        String dataHex = StringUtils.convertBytesToHex(subByte);
        Log.e(TAG, "recvData: len:  " + len[0] + "\tdata:  " + dataHex);
